...
- The main app logic is in `App.tsx` and the `carosigns/` directory, with UI components in `carosigns/components/` and data in `carosigns/data/`.
- The project uses TypeScript for data, hooks, and many UI components, with some UI logic still in JavaScript.
...
- **Practice Mode:**
  - Core logic in `carosigns/components/PracticeMode.js` and `carosigns/AppController.tsx`.
...
- **TypeScript for data, hooks, and many UI components, with some legacy JavaScript files.**
